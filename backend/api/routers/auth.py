...
from db.queries.auth import get_or_create_user
from models.pydantic.auth import Token, TelegramUserDataInDb
...
def telegram_auth(init_data: InitDataDep, settings: SettingsDep, db: SessionDep):
    ...
    user = get_or_create_user(init_data, db)
    access_token = create_access_token(data=user.model_dump(), settings=settings)
    refresh_token = create_refresh_token(data=user.model_dump(), settings=settings)
...
@router.get(
    "/me",
    status_code=200,
    response_model=TelegramUserDataInDb
)
